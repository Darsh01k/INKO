...
export function ShopShell() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#fcfcfd]">
      <header className="sticky top-0 z-40 border-b border-slate-200/70 bg-white/80 backdrop-blur-xl">
        <div className="mx-auto flex h-[64px] max-w-7xl items-center gap-3 px-4 sm:px-6 lg:px-8">
          <Link to="/shop/dashboard" className="flex items-center gap-2.5 font-semibold tracking-tight">
            ...
            <span className="hidden rounded-full border border-amber-200 bg-amber-50 px-2 py-0.5 text-[10px] font-medium tracking-widest text-amber-700 sm:inline">{t('appTagShop')}</span>
          </Link>
                ...
                {menuOpen && (
                  <>
                    ...
                    <div className="absolute right-0 z-40 mt-2 w-64 rounded-2xl border border-slate-200 bg-white p-2 shadow-lg">
                      ...
                      <Link to="/shop/profile" onClick={() => setMenuOpen(false)} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-sm hover:bg-slate-50"><User className="h-4 w-4 text-slate-500" /> {t('profile')}</Link>
                      <Link to="/shop/settings" onClick={() => setMenuOpen(false)} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-sm hover:bg-slate-50"><Settings className="h-4 w-4 text-slate-500" /> {t('settings')}</Link>
                      <div className="my-1 h-px bg-slate-100" />
        ...
        {mobileOpen && (
          <div className="border-t border-slate-200 bg-white px-4 py-3 lg:hidden">
            <nav className="grid gap-1">
              ...
              <Link to="/shop/profile" onClick={() => setMobileOpen(false)} className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-100"><User className="h-4 w-4" /> {t('profile')}</Link>
              <Link to="/shop/settings" onClick={() => setMobileOpen(false)} className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-100"><Settings className="h-4 w-4" /> {t('settings')}</Link>
      ...
      <footer className="mt-auto border-t border-slate-200 bg-white"><div className="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-6 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between sm:px-6 lg:px-8"><p>{t('footerShop').replace('{year}', String(new Date().getFullYear()))}</p><span className="inline-flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-emerald-500" /> {t('allSystems')}</span></div></footer>
    </div>
  )
}
