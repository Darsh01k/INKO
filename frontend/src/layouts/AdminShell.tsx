...
export function AdminShell() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#fcfcfd]">
      ...
      <footer className="mt-auto border-t border-slate-200 bg-white"><div className="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-6 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between sm:px-6 lg:px-8"><p>{t('footerAdmin').replace('{year}', String(new Date().getFullYear()))}</p><span className="inline-flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-emerald-500" /> {t('allSystems')}</span></div></footer>
    </div>
  )
}
