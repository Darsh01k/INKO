...
export default function MapPicker({ lat, lng, onPick }: { lat?: number | null, lng?: number | null, onPick: (r: PickResult)=>void }) {
  ...
  async function doSearch(){
    ...
    setLoading(true)
    try {
      const r = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(search)}&limit=1&addressdetails=1`)
      const j = await r.json()
      if (j?.[0]){
        const la = parseFloat(j[0].lat), ln = parseFloat(j[0].lon)
        setPickLat(String(la)); setPickLng(String(ln))
        setAddr(j[0].display_name || '')
  ...
  return (
    <div className="space-y-3 rounded-xl border border-slate-200 p-3 bg-slate-50/50">
      <div className="flex gap-2">
        <Input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search address, area, pincode…" onKeyDown={e=>{ if(e.key==='Enter'){ e.preventDefault(); doSearch() } }} />
