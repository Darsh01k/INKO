...
import { Printer, Mail, Lock, Phone, AlertCircle, ArrowRight, User } from 'lucide-react'
...
function canAccessShop(roles: string[]) {
  return roles.includes('SHOPKEEPER') || roles.includes('ADMIN') || roles.includes('SUPER_ADMIN')
}
...
export default function ShopLogin() {
  ...
  async function handlePasswordLogin(e: React.FormEvent) {
    ...
    try {
      ...
      if (!canAccessShop(user.roles)) { setError('This account is a customer. Please use Customer login.'); return }
      localStorage.setItem('inko.lastLoginRole', 'shop')
      navigate(next && next.startsWith('/shop') ? next : '/shop/dashboard', { replace: true })
    ...
    <main className="mesh-gradient flex min-h-screen items-center justify-center px-4 py-10">
      <div className="w-full max-w-[440px]">
        <div className="mb-8 flex flex-col items-center text-center">
          ...
          <h1 className="mt-4 text-[22px] font-bold tracking-tight text-slate-900">Shop sign in</h1>
          <p className="mt-1.5 text-sm text-slate-500">Manage queue, orders & shop operations</p>
        </div>
        <Card className="p-6 sm:p-7 shadow-lg">
          <div className="mb-2 inline-flex rounded-full border border-amber-200 bg-amber-50 px-2.5 py-1 text-xs font-medium text-amber-700">SHOP OS</div>
          ...
          {mode==='password' ? (
            <form onSubmit={handlePasswordLogin} className="space-y-4" noValidate>
              ...
              <Button type="submit" className="w-full" size="lg" loading={busy}>Sign in to shop <ArrowRight className="h-4 w-4" /></Button>
            </form>
          ) : (
          ...
          <div className="mt-6 rounded-xl border border-slate-200 bg-slate-50 p-3 text-center">
            <p className="text-xs text-slate-600">Are you a customer?</p><Link to="/login" className="inline-flex items-center gap-1 text-sm font-medium text-[oklch(0.55_0.20_260)] hover:underline"><User className="h-4 w-4"/> Customer sign in →</Link>
          </div>
          <p className="mt-3 text-center text-sm text-slate-500">New shop? <Link to="/register?type=shop" className="font-medium text-[oklch(0.55_0.20_260)] hover:underline">Create a shop account</Link></p>
        </Card>
        <p className="mt-6 text-center text-xs text-slate-400">Admins can also use Shop login — you’ll land in Shop dashboard</p>
      </div>
    </main>
  )
}
